...
- `GET /api/seckill/result?orderId=xxx`  
  �� `GET /api/seckill/result?userId=1001&productId=1`
- `GET /api/orders/:orderId`
- `GET /api/users/:userId/orders`
- `POST /api/seckill/stock/sync`��ȫ��ͬ����ɱ��滺�棩
- `POST /api/seckill/stock/sync/:productId`������Ʒͬ����

## 11. 分库分表（ShardingSphere-Proxy，选做）

已提供订单分库分表实现：

- 分库策略：按 `user_id % 2` 路由到 `ds_0 / ds_1`
- 分表策略：按 `order_id % 2` 路由到 `orders_0 / orders_1`
- 逻辑表名：`orders`
...
1. 调用 `POST /api/seckill/place-order` 连续下单多个用户。
2. 分别登录 `mysql-order-0`、`mysql-order-1` 查看 `orders_0`、`orders_1` 数据分布。
3. 查看 `GET /api/instance/info`，确认 `orderShardingEnabled=true`。
